...
export const BlobatarAvatar = ({ name, size = 50, active = false, animate = 'always', hue, tone, className = '' }) => {
  ...
  return (
    <span className={`blobatar-avatar ${className}`} style={{ width: size, height: size }} aria-label={`Avatar de ${avatarName}`}>
      <Blobatar ref={ref} name={avatarName} size={size} hue={hue} tone={tone} background="circle" animate={animate} />
    </span>
  );
};
