...
export function ConductIndicator({ user, onUserUpdate }) {
  ...
  async function addReport(event) {
    ...
    const report = { id: crypto.randomUUID(), reason: reason.trim(), createdAt: new Date().toISOString(), status: 'pending' }
  ...
  return <section className={`conduct-card conduct-card--${level}`} aria-label="Indicador de conducta">
    ...
    <details><summary>Historial de reportes ({reports.length})</summary>
      {reports.length ? <ul>{reports.map(report => <li key={report.id}><div><strong>{report.reason}</strong><small>{report.createdAt ? new Date(report.createdAt).toLocaleDateString('es') : ''} · {report.status === 'dismissed' ? 'Descartado' : 'Pendiente de revisión'}</small></div>{report.status !== 'dismissed' && <button type="button" disabled={busy} onClick={() => save(reports.map(item => item.id === report.id ? { ...item, status: 'dismissed' } : item), 'Reporte descartado.')}>Descartar</button>}</li>)}</ul> : <p>No hay reportes registrados.</p>}
    </details>
