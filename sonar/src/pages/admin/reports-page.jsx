...
export function ReportsPage({ users = [], reviews = [], onUserUpdate, onSendToModeration }) {
  ...
  return <section className="admin-panel reports-workspace" aria-labelledby="reports-title">
    ...
    {visible.map(report => <article className="admin-report" key={`${report.userId}-${report.id}`}><h3>{report.userName}</h3><p>{report.reason}</p>{report.contentSnapshot && <blockquote className="reports-notice"><strong>Contenido reportado</strong><p>{report.contentSnapshot}</p></blockquote>}<div className="admin-profile-facts"><span>Reportó: {users.find(user => sameId(user.id, report.reporterId))?.username || report.reporterName || 'No registrado'}</span><span>Contenido: {{ review: 'Reseña', comment: 'Comentario', reply: 'Respuesta' }[report.contentType] || 'Perfil'}</span><span>{report.createdAt ? new Date(report.createdAt).toLocaleString('es') : 'Sin fecha'}</span><span>{all.filter(item => sameId(item.userId, report.userId)).length} reportes acumulados</span><span>Estado: {{ pending: 'Pendiente', reviewed: 'Revisado', resolved: 'Resuelto' }[report.status]}</span></div><div className="report-actions">
      <div className="report-actions__group">
        {report.userId != null && <a className="report-action" href={`#usuarios?user=${encodeURIComponent(report.userId)}`}><UserRound size={16} aria-hidden="true" /><span>Ver usuario</span></a>}
      </div>
      <div className="report-actions__group report-actions__group--decisions">
        <button className="report-action report-action--moderate" disabled={busy || report.status === 'resolved'} onClick={() => action(report, 'reviewed', true)}><ShieldCheck size={16} aria-hidden="true" /><span>{report.standalone ? 'Marcar revisado' : 'Enviar a moderación'}</span></button>
