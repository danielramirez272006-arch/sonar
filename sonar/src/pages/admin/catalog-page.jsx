...
export function CatalogPage({ type }) {
  ...
  useEffect(() => {
    let active = true
    Promise.all([getCatalog(type), ['releases', 'vinyl'].includes(type) ? getCatalog('labels') : Promise.resolve([])]).then(([items, seals]) => {
      if (active) { setRows(items); setLabels(seals); setLoadError(''); setLoading(false) }
  ...
  function openEditor(row) { setEditor(row ? { ...row } : { status: 'draft', labelId: '' }); setError('') }
  ...
  return <section className="catalog-admin" aria-busy={loading || busy}>
    <header className="catalog-heading"><div><span className="eyebrow">GESTIÓN DEL CATÁLOGO</span><h1>{definition.title}</h1><p>Administra registros y prepara contenido para su publicación.</p></div><button className="primary-button" onClick={() => openEditor()} disabled={loading || !!loadError}><Plus size={17} aria-hidden="true" /> Nuevo registro</button></header>
    ...
    {loadError ? <div role="alert"><p>{loadError}</p><button onClick={() => { setLoading(true); setRefresh(value => value + 1) }}>Reintentar</button></div> : loading ? <p role="status">Cargando catálogo…</p> : !visible.length ? <div className="catalog-empty"><Disc3 size={32} aria-hidden="true" /><h2>{rows.length ? 'No hay coincidencias' : 'Tu catálogo empieza aquí'}</h2><p>{rows.length ? 'Cambia la búsqueda o el filtro.' : `Añade tu primer ${definition.singular}. Puedes guardarlo como borrador.`}</p></div> : <div className="catalog-list">{visible.map(row => <article className="catalog-row" key={row.id}><Disc3 size={24} aria-hidden="true" /><div><h2>{row.name || row.title}</h2><p>{[row.artist || row.country, row.releaseDate || row.year || row.founded, labels.find(label => String(label.id) === row.labelId)?.name].filter(Boolean).join(' · ')}</p></div><span className={`catalog-status catalog-status--${row.status}`}>{row.status === 'published' ? 'Publicado' : 'Borrador'}</span><div className="catalog-row-actions"><button onClick={() => openEditor(row)} aria-label={`Editar ${row.name || row.title}`}><Pencil size={15} aria-hidden="true" /> Editar</button><button onClick={() => { setDeleting(row); setError('') }} aria-label={`Eliminar ${row.name || row.title}`}><Trash2 size={15} aria-hidden="true" /> Eliminar</button></div></article>)}</div>}
    {editor && <Modal isOpen title={`${editor.id != null ? 'Editar' : 'Crear'} ${definition.singular}`} onClose={() => { if (!busy) setEditor(null) }}><form className="catalog-form" onSubmit={submit}><fieldset disabled={busy}>
      {definition.fields.map(([key, label, kind, required]) => <label key={key}>{label}{required ? ' *' : ''}{kind === 'textarea' ? <textarea required={!!required} maxLength={2000} rows={3} value={editor[key] || ''} onChange={e => setEditor({ ...editor, [key]: e.target.value })} /> : <input type={kind} required={!!required} maxLength={300} min={kind === 'number' ? 1800 : undefined} max={kind === 'number' ? new Date().getFullYear() + 5 : undefined} value={editor[key] ?? ''} onChange={e => setEditor({ ...editor, [key]: e.target.value })} />}</label>)}
      {['releases', 'vinyl'].includes(type) && <label>Sello discográfico<select value={editor.labelId || ''} onChange={e => setEditor({ ...editor, labelId: e.target.value })}><option value="">Sin sello asociado</option>{labels.map(label => <option key={label.id} value={label.id}>{label.name}</option>)}</select></label>}
      ...
      <p>El estado se guarda para su futura integración con el portal. Los campos con * son obligatorios.</p>{error && <p role="alert">{error}</p>}<div className="catalog-form-actions"><button type="button" onClick={() => setEditor(null)}>Cancelar</button><button className="primary-button" type="submit">{busy ? 'Guardando…' : 'Guardar registro'}</button></div>
    </fieldset></form></Modal>}
